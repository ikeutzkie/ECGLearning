...
public class HMM_For_Testing {
	 ...
	 *            A Hashtable that is the initial probability vector of the
	 *            states
	 * @param transitionMatrix
	 *            A Hashtable the transition matrix between the states
	 * @param emissionMatrix
	 *            A Hashtable that is the emission matrix between the states and
	 *            the observations
	 */

	public HMM_For_Testing(String name, Vector<String> states, Vector<String> observations,
	 ...
	 * @return [True/False] True/False which specifies if the matrix elements
	 *         are logically right or not
	 */
	 ...
	 *            An integer that specifies the number of observations in the
	 *            model
	 */
	 ...
	 * 
	 * @param emissionMatrix
	 *            Hashtable that is the emission matrix between the states and
	 *            the observations
	 */
	 ...
	 * @return A Double that is the value of the emission between the state and
	 *         the observation
	 */
	...
	/**
	 * Calculate the probability to obtain this sequence of states and
	 * observations which is the Evaluation of the model
	 * 
	 * @param states
	 ...
	 *         observations
	 * @throws Exception
	 *             The sizes of states and observations sequences must be the
	 *             same.
	 */
	...
	public Vector<Hashtable<String, Double>> calculateForwardProbabilities(Vector<String> states,
		...
		double sum1 = 0.0;
		for (int i = 0; i < states.size(); i++) {
			alpha.elementAt(0).put(states.get(i), this.getInitialProbability(states.get(i))
					* this.getEmissionValue(states.get(i), observations.get(0)));
			sum1 += this.getInitialProbability(states.get(i))
					* this.getEmissionValue(states.get(i), observations.get(0));
		}
		...
		for (int t = 1; t < states.size(); t++) {
			...
			for (int i = 0; i < states.size(); i++) {
				...
				for (int j = 0; j < states.size(); j++) {
					probability += alpha.elementAt(t - 1).get(states.get(j))
							* this.getTransitionValue(states.get(j), states.get(i));
				}
				alpha.elementAt(t).put(states.get(i),
	...
	public Vector<Hashtable<String, Double>> calculateBackwardProbabilities(Vector<String> states,
			Vector<String> observations) {
		...
		for (int t = states.size() - 2; t >= 0; t--) {
			...
			for (int i = 0; i < states.size(); i++) {
				double probability = 0.0;
				for (int j = 0; j < states.size(); j++) {
					probability += beta.elementAt(1).get(states.get(j))
